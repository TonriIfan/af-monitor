...
from monitoring.ml.features import extract_ppg_features
...
def _decode_header_strings(file_handle, dataset_path: str) -> list[str]:
    ...
    for ref in refs.flatten():
        result.append(_decode_char_dataset(file_handle[ref]))
    return result
...
def _build_ecg_index(ecg_path: Path) -> dict[str, np.ndarray | float | int]:
    with h5py.File(ecg_path, 'r') as handle:
        ...
        rr = np.asarray(handle['rr']).reshape(-1)
...
class Command(BaseCommand):
    help = '将 Zenodo 5815074 的 wrist PPG/ECG AF 数据转换为特征级训练 CSV。'
    ...
    def handle(self, *args, **options):
        ...
        if not any(data_dir.glob('*_PPG_*.mat')):
            raise CommandError(f'在 {data_dir} 未找到 PPG mat 文件。')

        patient_ids = sorted({path.name.split('_')[0] for path in data_dir.glob('*_PPG_*.mat')})
        ...
        with output_path.open('w', encoding='utf-8', newline='') as handle:
            writer = None
            for patient_id in patient_ids:
                ppg_files = sorted(data_dir.glob(f'{patient_id}_PPG_*.mat'))
                ...
                for ppg_path in ppg_files:
                    ...
                    for window_start in range(0, len(ppg_signal) - samples_per_window + 1, stride_samples):
                        ...
                        features, _ = extract_ppg_features(window_samples, ppg_rate)
        ...
        self.stdout.write(self.style.SUCCESS('Zenodo PPG AF 训练 CSV 已生成'))
